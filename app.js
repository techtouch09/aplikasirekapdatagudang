    ...
    <table class="report-table" border="1" cellpadding="6" style="width: 100%; border-collapse: collapse;">
        <thead>
            <tr style="background-color: #f8fafc;">
                <th>No</th><th>Tanggal</th><th>Kode</th><th>Nama Barang</th><th>Stok keluar</th><th>Qty</th><th>Sisa Stok</th><th>Keterangan</th>
            </tr>
        </thead>
        <tbody>
            ${dataMasukFiltered.length === 0 ? '<tr><td colspan="8" align="center">Tidak ada transaksi</td></tr>' : 
            dataMasukFiltered.map((x, i) => {
                ...
                <td align="center">${i + 1}</td>
                    <td>${formatTanggalIndo(x.tanggal)}</td>
                    ...
                    <td style="font-weight: bold; text-align: center;">${stokAkhir}</td>
                    <td class="col-ket">${x.ket}</td>
                </tr>`;
            }).join('')}
        </tbody>
    </table>
...
<div class="table-responsive">
    <table class="report-table" border="1" cellpadding="6" style="width: 100%; border-collapse: collapse;">
        <thead>
            <tr style="background-color: #f8fafc;">
                <th>No</th><th>Tanggal</th><th>Kode</th><th>Nama Barang</th><th>Qty</th><th>Stok Keluar</th><th>Sisa Stok</th><th>Keperluan</th>
            </tr>
        </thead>
        <tbody>
            ${dataKeluarFiltered.length === 0 ? '<tr><td colspan="8" align="center">Tidak ada transaksi</td></tr>' : 
...
function cetakReport() {
    // 1. Jalankan fungsi generate agar data masuk ke div 'printArea'
    generateReport(); 

    // 2. Beri jeda 300ms agar browser selesai me-render tabel ke DOM
    setTimeout(() => {
        window.print();
    }, 300);
}
...
function exportTable(type) {
    ...
    // CSS dasar agar file Word/Excel memiliki garis
    const style = `<style>
        table { border-collapse: collapse; width: 100%; }
        th, td { border: 1px solid black; padding: 5px; text-align: center; }
    </style>`;

    const fullHtml = `<html><head><meta charset="utf-8">${style}</head><body>${reportContent}</body></html>`;
