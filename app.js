...
function hitungStokBagus(kode, ignoreIndex = -1, type = '') {
    ...
    let totalRusak = db.rusak.filter((x, idx) => x.kode.trim() === kodeClean && !(type === 'rusak' && idx === ignoreIndex)).reduce((sum, i) => sum + i.qty, 0);
    return totalMasuk - totalKeluar - totalRusak;
}
...
document.getElementById('formRusak').addEventListener('submit', function(e) {
    ...
    const currentIdx = idx === "" ? -1 : parseInt(idx);
    const validStok = hitungStokBagus(kode, currentIdx, 'rusak');
    if(qty > validStok) { alert(`Gagal! Stok tidak mencukupi. Sisa stok bagus: ${validStok}`); return; }
...
function editRusak(i) {
    ...
    document.getElementById('bad_kode').value = db.bad_kode.value;
...
function refreshAllTables() {
    // 1. AMBIL KATA KUNCI & TANGGAL DARI INPUT FILTER PENCARIAN
    const keyword = document.getElementById('searchKeyword').value.toLowerCase().trim();
    const startDate = document.getElementById('searchStartDate').value;
    const endDate = document.getElementById('searchEndDate').value;

    // ================= PROSES FILTERING DATA MASTER =================
    ...
    // ================= PROSES FILTERING DATA TRANSAKSI (SINKRON) =================
    // Menyaring log masuk & keluar hanya untuk barang yang lolos dari filter keyword/tanggal master, 
    // ATAU jika transaksi itu sendiri mengandung kecocokan kode/tanggal secara langsung.
    ...
    // ================= 2. RENDER TABEL MINI: DETAIL PASOKAN MASUK (TERFILTER) =================
    ...
    // ================= 3. RENDER TABEL MINI: DETAIL ALOKASI KELUAR (TERFILTER) =================
    ...
    // ================= RENDER TABEL UTAMA DI TAB TRANSAKSI LAINNYA =================
    // (Tetap me-render data asli secara utuh di tab masing-masing supaya log utama tidak hilang)
...
function toggleMenu() {
    // Mengambil elemen nav dan tombol toggle
    const navMenu = document.getElementById('nav-menu');
    const menuBtn = document.querySelector('.menu-toggle');
    
    // Menambah / menghapus class aktif untuk memunculkan menu
